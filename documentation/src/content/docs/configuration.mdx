...
| ----------------- | -------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tsgolintVersion` | `string` | Optional | Tells `small-rules/require-safety-comment-for-type-assertion` which `oxlint-tsgolint` diagnostic range to expect for `typescript/no-unsafe-type-assertion`. |

`oxlint-tsgolint@7.0.2002` narrowed the report onto the `as Type` syntax. Older versions report the containing statement, so a `disable-next-line` above a multi-line object covers the diagnostic there but not on newer versions. Set `tsgolintVersion` to `"7.0.2002"` or later to require the disable to cover the assertion line itself.

When you omit `tsgolintVersion`, the rule infers it in this order: `node_modules/oxlint-tsgolint` installed version, then `pnpm-workspace.yaml`, then `package.json`. Invalid settings fall back to inference.
