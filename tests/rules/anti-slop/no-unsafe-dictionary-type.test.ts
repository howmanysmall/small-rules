...
describe("no-unsafe-dictionary-type", () => {
	ts.run("no-unsafe-dictionary-type", rule, {
		invalid: [
			...
			{ code: "type A = Record<string, string | unknown>;", errors: 1 },
			{ code: "interface Empty {} type A = Record<string, string | Empty>;", errors: [error] },
			...
			{ code: "type A = Readonly<Partial<Required<(Record<string, unknown>)>>>;", errors: 1 },
			...
			{
				code: "type Src<T> = T;\ntype A = { readonly [K in string]: Src<unknown> };",
				errors: 1,
			},
		],
		valid: [
